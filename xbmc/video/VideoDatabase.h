...
public:
  ...
  struct FileRecord
  {
    ...
    int64_t m_fileSize{0}; //!< file size in bytes, 0 if unknown (same convention as CVideoInfoTag)
  };
...
private:
  ...
  /*! \brief Determine the size of a file in bytes, unless already known.
   ...
   \param fileSize already known size in bytes, 0 if unknown
   \return the file size in bytes, 0 if it could not be determined
   */
  static int64_t GetFileSize(const std::string& filename, int64_t fileSize = 0);
};
